...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, num_light_idx, robust_train=False):
        ...
        for scan in scans:
            ...
            with open(os.path.join(self.datapath, scan, "cams/pair.txt")) as f:
                ...
                for view_idx in range(self.num_viewpoint):
                    ...
                    #for light_idx in range(self.num_light):
    ...
    def read_cam_file2(self, scan, filename):
        with open(filename) as f:
            lines = f.readlines()
            lines = [line.rstrip() for line in lines]
        # extrinsics: line [1,5), 4x4 matrix
    ...
    def read_depth_and_mask(self, filename, depth_min):
        ...
        depth_lr = np.squeeze(depth_lr, 2)
        mask = np.array(depth_lr >= depth_min, dtype=np.float32)
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                depth, mask = self.read_depth_and_mask(depth_filename_hr, depth_min)
