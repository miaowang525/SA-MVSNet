...
# os.environ['CUDA_VISIBLE_DEVICES'] = '0 '#看到那几张卡
#
# device_ids = [0]
...
parser.add_argument('--trainpath', default='G:/3d_model/code/data/dtu2', help='train datapath')
parser.add_argument('--valpath', help='validation datapath')
parser.add_argument('--trainlist', default='./lists/dtu2/train.txt', help='train list')
parser.add_argument('--vallist', default='./lists/dtu2/val.txt', help='validation list')

parser.add_argument('--epochs', type=int, default=10, help='number of epochs to train')
parser.add_argument('--lr', type=float, default=0.001, help='learning rate')
parser.add_argument('--lrepochs', type=str, default="8,10,12,14:2", help='epoch ids to downscale lr and the downscale rate')
parser.add_argument('--wd', type=float, default=0.0, help='weight decay')

parser.add_argument('--batch_size', type=int, default=1, help='train batch size')
parser.add_argument('--loadckpt', default=None, help='load a specific checkpoint')
parser.add_argument('--logdir', default='./outputs', help='the directory to save checkpoints/logs')
parser.add_argument('--resume', action='store_true', help='continue to train the model')

parser.add_argument('--summary_freq', type=int, default=20, help='print and summary frequency')
parser.add_argument('--save_freq', type=int, default=1, help='save checkpoint frequency')
parser.add_argument('--seed', type=int, default=1, metavar='S', help='random seed')
parser.add_argument("--num_light_idx", type=int, default=3, help="Number of light indexes in source images")
...
parser.add_argument('--evaluate_neighbors', nargs='+', type=int, default=[9, 9, 9],
        help='num of neighbors for adaptive matching cost aggregation of ada ptive evaluation on stages 1,2,3')
...
if args.dataset == 'dtu_yao':
    train_dataset = MVSDataset(args.trainpath, args.trainlist, "train", 5, num_light_idx=args.num_light_idx,
                               robust_train=True)
    test_dataset = MVSDataset(args.valpath, args.vallist, "val", 5, num_light_idx=args.num_light_idx,
                              robust_train=False)
...
if (args.mode == "train" and args.resume) or (args.mode == "test" and not args.loadckpt):
    ...
    ####################################
    # state_dict = torch.load(args.loadckpt)["model"]
    # del_keys = ['head.weight', 'head.bias', 'norm.weight', 'norm.bias']
    # for k in del_keys:
    #     del state_dict[k]
    # model.load_state_dict(state_dict, strict=False)
    # print(model.load_state_dict(state_dict, strict=False))
...
print("start at epoch {}".format(start_epoch + 1))
...
def train():
    ...
    #     optimizer, mode='min', factor=0.5, patience=3, verbose=True, cooldown=0, min_lr=0, eps=1e-08)

    for epoch_idx in range(start_epoch, args.epochs):
        print('Epoch {}:'.format(epoch_idx + 1))
        lr_scheduler.step()##################################################################学习率
        global_step = len(TrainImgLoader) * epoch_idx


        print("lr = ", optimizer.state_dict()['param_groups'][0]['lr'])
        writer2.add_scalar('lr', optimizer.state_dict()['param_groups'][0]['lr'], epoch_idx + 1)
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            print(
                'Epoch {}/{}, Iter {}/{}, train loss = {:.3f}, time = {:.3f}'.format(
                    epoch_idx + 1, args.epochs, batch_idx + 1, len(TrainImgLoader), loss, time.time() - start_time))
        print("Epoch {}/{}, train_loss = {}".format(epoch_idx + 1, args.epochs, avg_train_scalars.mean()["loss"]))
        writer2.add_scalar('train_loss', avg_train_scalars.mean()["loss"], epoch_idx + 1)
        writer2.add_scalar('train/train_loss', avg_train_scalars.mean()["loss"], epoch_idx + 1)

        writer2.add_scalar('train/train-depth-error-stage-0',
                           avg_train_scalars.mean()['abs_depth_error_refined_stage_0'], epoch_idx + 1)
        for i in range(1, 4):
            writer2.add_scalar('train/train-depth-error-stage-' + str(i),
                               avg_train_scalars.mean()['abs_depth_error_patchmatch_stage_' + str(i)], epoch_idx + 1)
        for t in range(0, 4):
            writer2.add_scalar('train/train-threshold-' + str(2**t)+'mm-error',
                               avg_train_scalars.mean()["thres" + str(2**t) + "mm_error"], epoch_idx + 1)
        ...
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            print('Epoch {}/{}, Iter {}/{}, test loss = {:.3f}, time = {:3f}'.format(
                epoch_idx + 1, args.epochs, batch_idx + 1, len(TestImgLoader), loss, time.time() - start_time))

        print("Epoch {}/{}, test_loss = {}".format(epoch_idx + 1, args.epochs, avg_test_scalars.mean()["loss"]))
        writer2.add_scalar('test_loss', avg_test_scalars.mean()["loss"], epoch_idx + 1)
        writer2.add_scalar('test/test_loss', avg_test_scalars.mean()["loss"], epoch_idx + 1)
        writer2.add_scalar('test/test-depth-error-stage-0',
                           avg_test_scalars.mean()['abs_depth_error_refined_stage_0'], epoch_idx + 1)
        for i in range(1, 4):
            writer2.add_scalar('test/test-depth-error-stage-' + str(i),
                               avg_test_scalars.mean()['abs_depth_error_patchmatch_stage_' + str(i)], epoch_idx + 1)
        for t in range(0, 4):
            writer2.add_scalar('test/test-threshold-' + str(2**t)+'mm-error',
                               avg_test_scalars.mean()["thres" + str(2**t) + "mm_error"], epoch_idx + 1)

        #lr_scheduler.step(avg_test_scalars.mean()["loss"])##################################################################学习率
...
def train_sample(sample, detailed_summary=False):
    ...
    # list1 = []
    # list1.append(
    #     AbsDepthError_metrics(depth_est['stage_0'], depth_gt['stage_0'], mask['stage_0'] > 0.5).cpu())
    # list1.append(
    #     AbsDepthError_metrics(depth_patchmatch['stage_3'][-1], depth_gt['stage_3'], mask['stage_3'] > 0.5).cpu())
    # list1.append(
    #     AbsDepthError_metrics(depth_patchmatch['stage_2'][-1], depth_gt['stage_2'], mask['stage_2'] > 0.5).cpu())
    # list1.append(
    #     AbsDepthError_metrics(depth_patchmatch['stage_1'][-1], depth_gt['stage_1'], mask['stage_1'] > 0.5).cpu())
    ...
    # list2 = []
    # for t in [1, 2, 4, 8]:
    #     list2.append(Thres_metrics(depth_est['stage_0'], depth_gt['stage_0'], mask['stage_0'] > 0.5, t).cpu())
