...
parser.add_argument('--usecpu', type=int, default=1, help='1 use cpu,0 use gpu if has')
...
def save_depth():
    ...
    model = PatchmatchNet(patchmatch_interval_scale=args.patchmatch_interval_scale,
                ...
                propagate_neighbors=args.propagate_neighbors, evaluate_neighbors=args.evaluate_neighbors).to(device)
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            if device==torch.device("cuda"):
                ...
                outputs = model(sample["imgs"], sample["proj_matrices"], 
