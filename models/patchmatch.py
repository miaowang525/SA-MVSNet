...
class PatchMatch(nn.Module):
    ...
    def __init__(
        ...
        if self.propagate_neighbors > 0:
            ...
            if not (self.stage == 1 and self.patchmatch_iteration == 1):
                # self.propa_conv = nn.Conv2d(in_channels=self.propa_num_feature, out_channels=2 * self.propagate_neighbors,
                #     kernel_size=3, stride=1, padding=self.dilation, dilation=self.dilation, bias=True, )
                # nn.init.constant_(self.propa_conv.weight, 0.0)
                # nn.init.constant_(self.propa_conv.bias, 0.0)
                self.propa_conv = attdconv(in_channels=self.propa_num_feature, out_channels=2 * self.propagate_neighbors,
                                           dilation=self.dilation)

                # adaptive spatial cost aggregation (adaptive evaluation)
        # self.eval_conv = nn.Conv2d(in_channels=self.propa_num_feature, out_channels=2 * self.evaluate_neighbors, kernel_size=3,
        #     stride=1, padding=self.dilation, dilation=self.dilation, bias=True,)
        # nn.init.constant_(self.eval_conv.weight, 0.0)
        # nn.init.constant_(self.eval_conv.bias, 0.0)
        self.eval_conv = attdconv(in_channels=self.propa_num_feature, out_channels=2 * self.evaluate_neighbors, dilation=self.dilation)
    ...
    def forward(
        ...
        if self.random_initialization:
            ...
            weight = weight / torch.sum(weight, dim=2).unsqueeze(2)  # [B,Ndepth,9,H,W]
...
class PixelwiseNet(nn.Module):
    ...
    def forward(self, x1: torch.Tensor) -> torch.Tensor:
        ...
        # x1: torch.Size([2, 8, 48, 64, 80])
        # x2: torch.Size([2, 48, 64, 80])
        # out: torch.Size([2, 48, 64, 80])
        # out2: torch.Size([2, 64, 80])
        # un: torch.Size([2, 1, 64, 80])
